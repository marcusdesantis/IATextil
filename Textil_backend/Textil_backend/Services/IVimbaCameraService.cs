...
public class VimbaCameraService : IVimbaCameraService, IDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Keep frames in natural buffer order (oldest→newest) so consecutive fabric strips stay
        // CONTINUOUS. To change the on-screen travel direction, rotate the whole stitched image
        // (see ImageProcessingService.TryWritePngAsync) — NOT by reversing frame order, which breaks
        // the fabric continuity.
